...
class SecurePremiumKeyConfig:
    ...
    _RAW_KEYS = {
        "ALPHA-TRADER-2024": {
            ...
            "created_at": "2024-01-01", "expires_at": "2026-12-31"
        },
        "BETA-INVEST-2024": {
...
<style>
    ...
    /* Remove top padding to eliminate white space */
    .css-1dp5vir {padding-top: 0rem !important;}
...
def apply_custom_css():
    ...
    /* ============================================ */
    /* INFO CARDS - Beautiful Content Cards        */
    /* ============================================ */
    ...
    .info-card h4, .info-card .card-title {
    ...
    .info-card p, .info-card .card-text {
    ...
    .info-card code, .info-card .formula-box {
    ...
    .info-card-purple li::before { color: #a78bfa; }
    .info-card-purple code { border-color: rgba(139, 92, 246, 0.3) !important; color: #c4b5fd !important; }
    ...
    .info-card-cyan li::before { color: #22d3ee; }
    .info-card-cyan code { border-color: rgba(34, 211, 238, 0.3) !important; color: #67e8f9 !important; }
    ...
    .info-card-green li::before { color: #10b981; }
    .info-card-green code { border-color: rgba(16, 185, 129, 0.3) !important; color: #6ee7b7 !important; }
    ...
    .info-card-amber li::before { color: #f59e0b; }
    .info-card-amber code { border-color: rgba(245, 158, 11, 0.3) !important; color: #fcd34d !important; }
    ...
    .info-card-rose li::before { color: #f43f5e; }
    .info-card-rose code { border-color: rgba(244, 63, 94, 0.3) !important; color: #fda4af !important; }
    
    /* Formula Card - Special styling for math formulas */
    .formula-card {
    ...
    .formula-card .formula-desc strong {
        color: #e2e8f0;
    }
    ...
    /* ============================================ */
    /* SCROLLBAR STYLING - White & Always Visible  */
    /* ============================================ */
    ...
    html {
        ...
        scrollbar-color: rgba(255, 255, 255, 0.5) #1e293b;
    }
    
    /* Info/Alert Box Styling - Make text more visible */
    .stAlert {
    ...
    /* Target info icon and text */
    .stAlert svg {
        fill: #60a5fa !important;
    }
    
    /* Streamlit info box text */
    ...
    /* ============================================ */
    /* FIX FOR DIM MARKDOWN TEXT THROUGHOUT THE APP */
    /* ============================================ */
    
    /* Main markdown text - make it bright and readable */
    ...
    /* Bold text should be even brighter */
    ...
    /* All paragraph text in the main content area */
    ...
    /* ============================================ */
    /* TRAINING & CV RESULTS STYLING               */
    /* ============================================ */
    
    /* Metric cards - make values bright and visible */
    [data-testid="stMetric"] {
        background: linear-gradient(135deg, rgba(99, 102, 241, 0.1), rgba(99, 102, 241, 0.02)) !important;
    ...
    /* Success/Warning/Error/Info messages - enhanced styling */
    [data-testid="stAlert"] {
        border-radius: 12px !important;
        padding: 16px 20px !important;
    }
    ...
    .training-result-card .result-label {
        color: #6ee7b7;
        font-size: 13px;
        font-weight: 600;
        text-transform: uppercase;
    ...
    .training-result-card .result-value {
        color: #f1f5f9;
        font-size: 24px;
        font-weight: 700;
        margin-top: 4px;
    ...
    .cv-result-card .cv-label {
        color: #c4b5fd;
        font-size: 13px;
        font-weight: 600;
    }
    
    .cv-result-card .cv-value {
        color: #f1f5f9;
        font-size: 22px;
        font-weight: 700;
    }
...
FREE_TIER_CONFIG = {
    ...
    # Training Lab Restrictions
    "max_trainings_per_day": 3,
    "max_training_epochs": 30,
    "free_training_models": ["xgboost", "sklearn_ensemble"],
    # Cross-Validation Lab Restrictions
    "max_cv_runs_per_day": 3,
    "max_cv_folds": 3,
    "free_cv_methods": ["time_series"],
}
...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        frontend_icon = "✅" if PREMIUMVER_IMPORTED else "❌"
        backend_icon = "✅" if ENHPROG_IMPORTED else "❌"
        api_icon = "✅" if FMP_API_KEY else "❌"
        
        st.markdown(f"""
        <div class="backend-status" style="border-color: {status_color};">
            <strong style="color: #e2e8f0;">{status_text}</strong><br>
            <small style="color: #94a3b8;">
                Frontend: {frontend_icon}<br>
                Backend: {backend_icon}<br>
                API: {api_icon}
            </small>
        </div>
        """, unsafe_allow_html=True)
...
def render_analytics():
    ...
    if PREMIUMVER_IMPORTED:
        st.success("🟢 Using Enhanced Analytics Suite")
...
def render_training_labs():
    ...
    if ENHPROG_IMPORTED:
        st.success("🟢 **Full Backend Connected** — All advanced features available")
...
def render_model_training_lab(lab):
    """Interactive Model Training Lab with free tier restrictions"""
    
    # Define free and premium models
    FREE_TRAINING_MODELS = ['xgboost', 'sklearn_ensemble']
    PREMIUM_TRAINING_MODELS = ['advanced_transformer', 'cnn_lstm', 'enhanced_tcn', 'enhanced_nbeats', 'lstm_gru_ensemble']
    
    # Free tier limitations
    FREE_MAX_EPOCHS = 30
    FREE_MAX_TRAININGS_PER_DAY = 3
    
    # Track daily training count
    if 'training_count_today' not in st.session_state:
        st.session_state.training_count_today = 0
        st.session_state.training_date = datetime.now().strftime('%Y-%m-%d')
    
    # Reset count if new day
    current_date = datetime.now().strftime('%Y-%m-%d')
    if st.session_state.training_date != current_date:
        st.session_state.training_count_today = 0
        st.session_state.training_date = current_date
    ...
    with col1:
        ...
        # Show tier badge and limitations
        if is_premium:
            st.markdown("""
            <div style="background: linear-gradient(135deg, rgba(245, 158, 11, 0.2), rgba(245, 158, 11, 0.05));
                        border: 1px solid rgba(245, 158, 11, 0.4); border-radius: 10px; padding: 12px; margin-bottom: 16px;">
                <div style="color: #fcd34d; font-weight: 600;">👑 Premium Access</div>
                <div style="color: #e2e8f0; font-size: 13px; margin-top: 4px;">All models • Unlimited epochs • Unlimited training</div>
            </div>
            """, unsafe_allow_html=True)
        else:
            remaining_trainings = max(0, FREE_MAX_TRAININGS_PER_DAY - st.session_state.training_count_today)
            st.markdown(f"""
            <div style="background: linear-gradient(135deg, rgba(99, 102, 241, 0.15), rgba(99, 102, 241, 0.05));
                        border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 10px; padding: 12px; margin-bottom: 16px;">
                <div style="color: #a5b4fc; font-weight: 600;">🆓 Free Tier</div>
                <div style="color: #94a3b8; font-size: 13px; margin-top: 4px;">
                    Basic models only • Max {FREE_MAX_EPOCHS} epochs • {remaining_trainings}/{FREE_MAX_TRAININGS_PER_DAY} trainings left today
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if is_premium:
            ...
            model_options = {
                'xgboost': '🎯 XGBoost (Free)',
                'sklearn_ensemble': '📦 Sklearn Ensemble (Free)',
            }
            # Show locked premium models
            st.markdown("**🔒 Premium Models:**")
            st.markdown("""
            <div style="color: #64748b; font-size: 13px; padding: 8px; background: rgba(0,0,0,0.2); border-radius: 8px; margin-bottom: 12px;">
                🧠 Transformer • 🔗 CNN-LSTM • 📊 TCN • 📈 N-BEATS • 🔄 LSTM-GRU
            </div>
            """, unsafe_allow_html=True)
        ...
        # Epoch limits based on tier
        max_epochs = 100 if is_premium else FREE_MAX_EPOCHS
        epochs = st.slider("Training Epochs", 10, max_epochs, min(50, max_epochs), key="train_epochs")
        
        if not is_premium and epochs == FREE_MAX_EPOCHS:
            st.caption(f"⚠️ Free tier limited to {FREE_MAX_EPOCHS} epochs. Upgrade for more!")
        
        learning_rate = st.select_slider("Learning Rate", 
                                          options=[0.0001, 0.0005, 0.001, 0.005, 0.01],
                                          value=0.001, key="train_lr")
        
        # CV option - limited for free tier
        if is_premium:
            use_cv = st.checkbox("Enable Cross-Validation", value=True, key="train_cv")
        else:
            use_cv = False
            st.checkbox("Enable Cross-Validation", value=False, disabled=True, key="train_cv",
                       help="Cross-validation during training requires Premium")
        
        # Check if free user has reached daily limit
        can_train = True
        if not is_premium and st.session_state.training_count_today >= FREE_MAX_TRAININGS_PER_DAY:
            can_train = False
            st.error(f"❌ Daily training limit reached ({FREE_MAX_TRAININGS_PER_DAY}/day). Upgrade to Premium for unlimited training!")
        
        if st.button("🚀 Start Training", use_container_width=True, key="start_training", disabled=not can_train):
            # Increment training count for free users
            if not is_premium:
                st.session_state.training_count_today += 1
            ...
            with st.spinner(f"Training {model_options[selected_model]}..."):
                ...
                for epoch in range(epochs):
                    ...
                    if epoch % 5 == 0:
                        ...
                        <div style="background: linear-gradient(135deg, rgba(99, 102, 241, 0.15), rgba(99, 102, 241, 0.05)); 
                                    border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 10px; padding: 12px 16px;">
                            <div style="color: #a5b4fc; font-size: 13px; font-weight: 600;">Epoch {epoch+1}/{epochs}</div>
                            <div style="display: flex; gap: 24px; margin-top: 8px;">
                                <div>
                                    <span style="color: #94a3b8; font-size: 12px;">Train Loss:</span>
                                    <span style="color: #6366f1; font-size: 16px; font-weight: 600; margin-left: 6px;">{loss:.6f}</span>
                                </div>
                                <div>
                                    <span style="color: #94a3b8; font-size: 12px;">Val Loss:</span>
                                    <span style="color: #ef4444; font-size: 16px; font-weight: 600; margin-left: 6px;">{val_loss:.6f}</span>
                                </div>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        if st.session_state.get('models_trained'):
            ...
            for model_name, info in st.session_state.models_trained.items():
                ...
                <div class="training-result-card">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div class="result-label">✅ {model_name}</div>
                            <div style="color: #94a3b8; font-size: 13px; margin-top: 4px;">Trained on {info['ticker']}</div>
                        </div>
                        <div style="text-align: right;">
                            <div style="color: #6ee7b7; font-size: 12px;">Final Loss</div>
                            <div class="result-value" style="font-size: 20px;">{info['final_loss']:.6f}</div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
def render_cross_validation_lab(lab):
    """Interactive Cross-Validation Workshop with free tier restrictions"""
    
    # Free tier limitations
    FREE_MAX_FOLDS = 3
    FREE_CV_METHODS = ['time_series']  # Only basic time series split for free
    PREMIUM_CV_METHODS = ['time_series', 'walk_forward', 'purged']
    FREE_MAX_CV_RUNS_PER_DAY = 3
    
    # Track daily CV count
    if 'cv_count_today' not in st.session_state:
        st.session_state.cv_count_today = 0
        st.session_state.cv_date = datetime.now().strftime('%Y-%m-%d')
    
    # Reset count if new day
    current_date = datetime.now().strftime('%Y-%m-%d')
    if st.session_state.cv_date != current_date:
        st.session_state.cv_count_today = 0
        st.session_state.cv_date = current_date
    ...
    with col1:
        ...
        if is_premium:
            ...
            <div style="background: linear-gradient(135deg, rgba(245, 158, 11, 0.2), rgba(245, 158, 11, 0.05));
                        border: 1px solid rgba(245, 158, 11, 0.4); border-radius: 10px; padding: 12px; margin-bottom: 16px;">
                <div style="color: #fcd34d; font-weight: 600;">👑 Premium Access</div>
                <div style="color: #e2e8f0; font-size: 13px; margin-top: 4px;">All CV methods • Up to 10 folds • Unlimited runs</div>
            </div>
            """, unsafe_allow_html=True)
        else:
            remaining_runs = max(0, FREE_MAX_CV_RUNS_PER_DAY - st.session_state.cv_count_today)
            st.markdown(f"""
            <div style="background: linear-gradient(135deg, rgba(99, 102, 241, 0.15), rgba(99, 102, 241, 0.05));
                        border: 1px solid rgba(99, 102, 241, 0.3); border-radius: 10px; padding: 12px; margin-bottom: 16px;">
                <div style="color: #a5b4fc; font-weight: 600;">🆓 Free Tier</div>
                <div style="color: #94a3b8; font-size: 13px; margin-top: 4px;">
                    Time Series Split only • Max {FREE_MAX_FOLDS} folds • {remaining_runs}/{FREE_MAX_CV_RUNS_PER_DAY} runs left today
                </div>
            </div>
            """, unsafe_allow_html=True)
        
        # CV Method selection based on tier
        if is_premium:
            cv_methods = PREMIUM_CV_METHODS
            ...
            cv_method = st.selectbox("CV Method", 
                                      FREE_CV_METHODS,
                                      format_func=lambda x: '📊 Time Series Split (Free)',
                                      key="cv_method")
            # Show locked premium methods
            st.markdown("**🔒 Premium CV Methods:**")
            st.markdown("""
            <div style="color: #64748b; font-size: 13px; padding: 8px; background: rgba(0,0,0,0.2); border-radius: 8px; margin-bottom: 12px;">
                🚶 Walk-Forward Validation • 🛡️ Purged CV (prevents leakage)
            </div>
            """, unsafe_allow_html=True)
        
        # Number of folds - limited for free tier
        max_folds = 10 if is_premium else FREE_MAX_FOLDS
        n_folds = st.slider("Number of Folds", 2, max_folds, min(5, max_folds), key="cv_folds")
        
        if not is_premium and n_folds == FREE_MAX_FOLDS:
            st.caption(f"⚠️ Free tier limited to {FREE_MAX_FOLDS} folds. Upgrade for more!")
        ...
        # Check if free user has reached daily limit
        can_run_cv = True
        if not is_premium and st.session_state.cv_count_today >= FREE_MAX_CV_RUNS_PER_DAY:
            can_run_cv = False
            st.error(f"❌ Daily CV limit reached ({FREE_MAX_CV_RUNS_PER_DAY}/day). Upgrade to Premium for unlimited runs!")
        
        if st.button("🔬 Run Cross-Validation", use_container_width=True, key="run_cv", disabled=not can_run_cv):
            # Increment CV count for free users
            if not is_premium:
                st.session_state.cv_count_today += 1
            ...
            with st.spinner("Running cross-validation..."):
                ...
                <div class="cv-result-card">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div class="cv-label">✅ Cross-Validation Complete</div>
                            <div style="color: #e2e8f0; margin-top: 8px;">Method: <strong>{cv_method.replace('_', ' ').title()}</strong></div>
                        </div>
                        <div style="text-align: right;">
                            <div style="color: #c4b5fd; font-size: 12px;">Mean MSE ± Std</div>
                            <div class="cv-value">{cv_results['mean_mse']:.6f} <span style="font-size: 14px; color: #a5b4fc;">± {cv_results['std_mse']:.6f}</span></div>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
def render_ml_models():
    ...
    # Show backend status
    if ENHPROG_IMPORTED:
        st.success("🟢 All models available from backend")
    elif PREMIUMVER_IMPORTED:
        st.success("🟢 Models available from frontend")
    else:
...
def render_cross_validation():
    ...
    if PREMIUMVER_IMPORTED:
        st.success("🟢 Using Real Cross-Validation Engine")
    ...
    with col2:
        ...
        if st.session_state.cross_validation_results:
            ...
            <div class="best-model-card">
                ...
                <div class="model-name">{cv.get('best_model', 'N/A')}</div>
                <div class="score">MSE Score: <strong>{cv.get('best_score', 0):.6f}</strong></div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        with model_tabs[0]:
            ...
            with col1:
                ...
                <div class="info-card info-card-purple">
                    <h4>🧠 Advanced Transformer Architecture</h4>
                    <p>The Transformer model uses <strong>self-attention mechanisms</strong> to understand 
                    relationships between all time points in a sequence simultaneously.</p>
                    
                    <div class="formula-title" style="color: #c4b5fd; font-weight: 600; margin: 16px 0 12px 0;">Key Components:</div>
                    <div class="component-list">
                        <div class="component-item" style="border-left-color: #a78bfa;">
                            <span class="icon">📍</span>
                            <span class="text"><strong>Positional Encoding:</strong> Adds sequence position information</span>
                        </div>
                        <div class="component-item" style="border-left-color: #a78bfa;">
                            <span class="icon">🎯</span>
                            <span class="text"><strong>Multi-Head Attention:</strong> 8 parallel attention heads capture different patterns</span>
                        </div>
                        <div class="component-item" style="border-left-color: #a78bfa;">
                            <span class="icon">⚡</span>
                            <span class="text"><strong>Feed-Forward Network:</strong> Non-linear transformations</span>
                        </div>
                        <div class="component-item" style="border-left-color: #a78bfa;">
                            <span class="icon">📊</span>
                            <span class="text"><strong>Layer Normalization:</strong> Stabilizes training</span>
                        </div>
                    </div>
                    
                    <div class="formula-title" style="color: #c4b5fd; font-weight: 600; margin: 16px 0 12px 0;">Why it works for trading:</div>
                    <ul>
                        <li>Captures long-range dependencies (e.g., weekly patterns affecting daily prices)</li>
                        <li>Handles variable-length sequences efficiently</li>
                        <li>Learns complex temporal relationships</li>
                    </ul>
                </div>
                """, unsafe_allow_html=True)
        ...
        with model_tabs[1]:
            ...
            with col1:
                ...
                <div class="info-card info-card-cyan">
                    <h4>🔗 CNN-LSTM Hybrid Architecture</h4>
                    <p>This model combines <strong>Convolutional Neural Networks</strong> for local pattern 
                    extraction with <strong>LSTM</strong> for temporal sequence modeling.</p>
                    
                    <div class="formula-title" style="color: #67e8f9; font-weight: 600; margin: 16px 0 12px 0;">Processing Flow:</div>
                    <div class="component-list">
                        <div class="component-item" style="border-left-color: #22d3ee;">
                            <span class="icon">1️⃣</span>
                            <span class="text"><strong>Conv1D Layers:</strong> Extract local features (candlestick patterns)</span>
                        </div>
                        <div class="component-item" style="border-left-color: #22d3ee;">
                            <span class="icon">2️⃣</span>
                            <span class="text"><strong>Batch Normalization:</strong> Normalize activations</span>
                        </div>
                        <div class="component-item" style="border-left-color: #22d3ee;">
                            <span class="icon">3️⃣</span>
                            <span class="text"><strong>LSTM Layers:</strong> Model temporal dependencies</span>
                        </div>
                        <div class="component-item" style="border-left-color: #22d3ee;">
                            <span class="icon">4️⃣</span>
                            <span class="text"><strong>Attention Layer:</strong> Focus on important time steps</span>
                        </div>
                        <div class="component-item" style="border-left-color: #22d3ee;">
                            <span class="icon">5️⃣</span>
                            <span class="text"><strong>Dense Layers:</strong> Final prediction</span>
                        </div>
                    </div>
                    
                    <div class="formula-title" style="color: #67e8f9; font-weight: 600; margin: 16px 0 12px 0;">Why it works for trading:</div>
                    <ul>
                        <li>CNNs detect chart patterns (head & shoulders, triangles)</li>
                        <li>LSTMs remember important historical events</li>
                        <li>Attention highlights critical turning points</li>
                    </ul>
                </div>
                """, unsafe_allow_html=True)
        ...
        with model_tabs[2]:
            ...
            <div class="info-card info-card-green">
                <h4>📊 Temporal Convolutional Network (TCN)</h4>
                <p>TCN uses <strong>dilated causal convolutions</strong> to capture patterns across different 
                time scales while maintaining temporal causality.</p>
                
                <div class="formula-title" style="color: #6ee7b7; font-weight: 600; margin: 16px 0 12px 0;">Key Features:</div>
                <div class="component-list">
                    <div class="component-item" style="border-left-color: #10b981;">
                        <span class="icon">🔄</span>
                        <span class="text"><strong>Dilated Convolutions:</strong> Exponentially increasing receptive field</span>
                    </div>
                    <div class="component-item" style="border-left-color: #10b981;">
                        <span class="icon">⏱️</span>
                        <span class="text"><strong>Causal Padding:</strong> No future information leakage</span>
                    </div>
                    <div class="component-item" style="border-left-color: #10b981;">
                        <span class="icon">🔗</span>
                        <span class="text"><strong>Residual Connections:</strong> Enable deep networks</span>
                    </div>
                </div>
                
                <div class="formula-content" style="background: rgba(16, 185, 129, 0.1); border-color: rgba(16, 185, 129, 0.3);">
                    <strong style="color: #6ee7b7;">Dilation Pattern:</strong> 1, 2, 4, 8, 16, 32...
                </div>
                
                <p style="margin-top: 12px;">This allows the network to see patterns at multiple time scales 
                (hourly, daily, weekly) simultaneously.</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with model_tabs[3]:
            ...
            <div class="info-card info-card-amber">
                <h4>⚡ N-BEATS (Neural Basis Expansion)</h4>
                <p>N-BEATS is specifically designed for time series forecasting with 
                <strong>interpretable components</strong>.</p>
                
                <div class="formula-title" style="color: #fcd34d; font-weight: 600; margin: 16px 0 12px 0;">Architecture:</div>
                <div class="component-list">
                    <div class="component-item" style="border-left-color: #f59e0b;">
                        <span class="icon">📦</span>
                        <span class="text"><strong>Stacked Blocks:</strong> Each block learns different aspects</span>
                    </div>
                    <div class="component-item" style="border-left-color: #f59e0b;">
                        <span class="icon">📈</span>
                        <span class="text"><strong>Trend Block:</strong> Captures overall direction</span>
                    </div>
                    <div class="component-item" style="border-left-color: #f59e0b;">
                        <span class="icon">🔁</span>
                        <span class="text"><strong>Seasonality Block:</strong> Captures periodic patterns</span>
                    </div>
                    <div class="component-item" style="border-left-color: #f59e0b;">
                        <span class="icon">🎯</span>
                        <span class="text"><strong>Backcast/Forecast:</strong> Reconstructs past & predicts future</span>
                    </div>
                </div>
                
                <div class="formula-title" style="color: #fcd34d; font-weight: 600; margin: 16px 0 12px 0;">Why it's powerful:</div>
                <ul>
                    <li>Interpretable outputs (can see trend vs seasonality)</li>
                    <li>No need for feature engineering</li>
                    <li>State-of-the-art on many benchmarks</li>
                </ul>
            </div>
            """, unsafe_allow_html=True)
        ...
        with indicator_tabs[0]:
            ...
            with col2:
                ...
                <div class="info-card info-card-cyan">
                    ...
                    <p>Smooth out price data to <strong>identify trends</strong>.</p>
                    
                    <div class="component-list">
                        <div class="component-item" style="border-left-color: #6366f1; padding: 6px 12px;">
                            <span class="text"><strong>SMA:</strong> Simple average of last N prices</span>
                        </div>
                        <div class="component-item" style="border-left-color: #10b981; padding: 6px 12px;">
                            <span class="text"><strong>EMA:</strong> Weighted average (recent prices matter more)</span>
                        </div>
                    </div>
                    
                    <div class="formula-title" style="color: #67e8f9; font-weight: 600; margin: 12px 0 8px 0; font-size: 14px;">Trading Signals:</div>
                    <ul>
                        <li>Price > MA = Bullish</li>
                        <li>Golden Cross (SMA20 > SMA50) = Buy</li>
                        <li>Death Cross (SMA20 < SMA50) = Sell</li>
                    </ul>
                    
                    <div style="background: rgba(34, 211, 238, 0.1); border-radius: 8px; padding: 10px 14px; margin-top: 12px;">
                        <span style="color: #67e8f9; font-size: 12px;"><strong>Used:</strong> SMA (5, 10, 20, 50, 100, 200) • EMA (5, 10, 20, 50) • Price-to-MA ratios</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with indicator_tabs[1]:
            ...
            with col2:
                ...
                <div class="info-card info-card-purple">
                    ...
                    <p>Measures momentum on a <strong>0-100 scale</strong>.</p>
                    
                    <div class="formula-content" style="border-color: rgba(139, 92, 246, 0.3); color: #c4b5fd;">
RSI = 100 - (100 / (1 + RS))
RS = Avg Gain / Avg Loss
                    </div>
                    
                    <div class="formula-title" style="color: #c4b5fd; font-weight: 600; margin: 12px 0 8px 0; font-size: 14px;">Interpretation:</div>
                    <div class="component-list">
                        <div class="component-item" style="border-left-color: #ef4444; padding: 6px 12px;">
                            <span class="text"><strong>RSI > 70</strong> = Overbought (potential sell)</span>
                        </div>
                        <div class="component-item" style="border-left-color: #10b981; padding: 6px 12px;">
                            <span class="text"><strong>RSI < 30</strong> = Oversold (potential buy)</span>
                        </div>
                        <div class="component-item" style="border-left-color: #6366f1; padding: 6px 12px;">
                            <span class="text"><strong>RSI = 50</strong> = Neutral</span>
                        </div>
                    </div>
                    
                    <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(139, 92, 246, 0.2);">
                        <span style="color: #a5b4fc; font-size: 13px;"><strong>Other Momentum Indicators:</strong> MACD, Stochastic, Williams %R, ROC</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with indicator_tabs[2]:
            ...
            with col2:
                ...
                <div class="info-card info-card-amber">
                    ...
                    <p>Measures <strong>volatility</strong> and identifies overbought/oversold conditions.</p>
                    
                    <div class="formula-title" style="color: #fcd34d; font-weight: 600; margin: 12px 0 8px 0; font-size: 14px;">Components:</div>
                    <div class="component-list">
                        <div class="component-item" style="border-left-color: #6366f1; padding: 6px 12px;">
                            <span class="text"><strong>Middle Band</strong> = SMA(20)</span>
                        </div>
                        <div class="component-item" style="border-left-color: #ef4444; padding: 6px 12px;">
                            <span class="text"><strong>Upper Band</strong> = SMA + 2σ</span>
                        </div>
                        <div class="component-item" style="border-left-color: #10b981; padding: 6px 12px;">
                            <span class="text"><strong>Lower Band</strong> = SMA - 2σ</span>
                        </div>
                    </div>
                    
                    <div class="formula-title" style="color: #fcd34d; font-weight: 600; margin: 12px 0 8px 0; font-size: 14px;">Interpretation:</div>
                    <ul>
                        <li>Price near upper band = Overbought</li>
                        <li>Price near lower band = Oversold</li>
                        <li>Band squeeze = Breakout coming</li>
                        <li>Band expansion = High volatility</li>
                    </ul>
                    
                    <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(245, 158, 11, 0.2);">
                        <span style="color: #fcd34d; font-size: 13px;"><strong>Other Volatility Indicators:</strong> ATR, Standard Deviation, Keltner Channels</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    with tab5:
        ...
        with col2:
            st.markdown("""
            <div class="info-card">
                <h4>📊 VaR Calculation Methods</h4>
                
                <div class="component-list">
                    <div class="component-item">
                        <span class="icon">1️⃣</span>
                        <span class="text"><strong>Historical VaR</strong><br>
                        <span style="color: #94a3b8; font-size: 13px;">Use actual historical returns. Simple but assumes history repeats.</span></span>
                    </div>
                    <div class="component-item">
                        <span class="icon">2️⃣</span>
                        <span class="text"><strong>Parametric VaR</strong><br>
                        <span style="color: #94a3b8; font-size: 13px;">Assume normal distribution: VaR = μ - (z × σ). Fast but may miss fat tails.</span></span>
                    </div>
                    <div class="component-item">
                        <span class="icon">3️⃣</span>
                        <span class="text"><strong>Monte Carlo VaR</strong><br>
                        <span style="color: #94a3b8; font-size: 13px;">Simulate thousands of scenarios. Most flexible but computationally expensive.</span></span>
                    </div>
                </div>
                
                <div style="background: rgba(99, 102, 241, 0.15); border-radius: 8px; padding: 12px 16px; margin-top: 16px;">
                    <strong style="color: #a5b4fc;">This system uses:</strong> 
                    <span style="color: #e2e8f0;">Historical + Monte Carlo blend</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
Where:
  ...
  σp = Portfolio std deviation
                ...
                <div class="formula-desc">
                    Measures <strong>risk-adjusted returns</strong>. Higher is better (>1 is good, >2 is excellent).
                </div>
            </div>
            
            <div class="formula-card" style="border-color: rgba(139, 92, 246, 0.3);">
                <div class="formula-title">📈 Sortino Ratio</div>
                <div class="formula-content" style="border-color: rgba(139, 92, 246, 0.2);">
Sortino = (Rp - Rf) / σd
...
Where:
  σd = Downside deviation only
                ...
                <div class="formula-desc">
                    Better than Sharpe for <strong>asymmetric returns</strong>. Only penalizes downside volatility.
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with formula_col2:
            st.markdown("""
            <div class="formula-card" style="border-color: rgba(245, 158, 11, 0.3);">
                <div class="formula-title">📉 Maximum Drawdown</div>
                <div class="formula-content" style="border-color: rgba(245, 158, 11, 0.2);">
MDD = (Peak - Trough) / Peak

Measures worst historical decline
                </div>
                <div class="formula-desc">
                    <strong>Important for risk tolerance.</strong> Shows the maximum loss from a peak before recovery.
                </div>
            </div>
            
            <div class="formula-card" style="border-color: rgba(16, 185, 129, 0.3);">
                <div class="formula-title">⚖️ Calmar Ratio</div>
                <div class="formula-content" style="border-color: rgba(16, 185, 129, 0.2);">
Calmar = Annual Return / Max Drawdown

Measures return per unit of drawdown risk
                </div>
                <div class="formula-desc">
                    <strong>Higher is better</strong> (>1 is good). Balances returns against worst-case scenarios.
                </div>
            </div>
            """, unsafe_allow_html=True)
...
def render_settings():
    """Render comprehensive Settings page with modern features"""
    st.markdown("## ⚙️ Settings & Preferences")
    
    # Create tabs for organized settings
    tabs = st.tabs(["🔗 System Status", "🎨 Display Preferences", "📊 Account & Usage", "⚙️ Advanced Settings", "ℹ️ About"])
    ...
    with tabs[0]:
        ...
        with col1:
            st.markdown("#### Backend Components")
            frontend_icon = "✅" if PREMIUMVER_IMPORTED else "❌"
            backend_icon = "✅" if ENHPROG_IMPORTED else "❌"
            full_system_icon = "✅" if FULL_BACKEND else "❌"
            
            st.metric("Frontend Module", "Active" if PREMIUMVER_IMPORTED else "Inactive", 
                     delta="Operational" if PREMIUMVER_IMPORTED else "Check logs")
            st.metric("Backend Module", "Active" if ENHPROG_IMPORTED else "Inactive",
                     delta="Operational" if ENHPROG_IMPORTED else "Check logs")
            st.metric("Full System", "Online" if FULL_BACKEND else "Offline",
                     delta="Ready" if FULL_BACKEND else "Limited mode")
        
        with col2:
            st.markdown("#### API Connections")
            api_icon = "✅" if FMP_API_KEY else "❌"
            
            st.metric("FMP API", "Connected" if FMP_API_KEY else "Not Connected",
                     delta="Live data" if FMP_API_KEY else "Using cache")
            st.metric("ML Models", f"{8 if FULL_BACKEND else 2} Available",
                     delta="Full suite" if FULL_BACKEND else "Basic only")
            st.metric("Data Cache", "Active", delta="Fresh")
        
        with col3:
            st.markdown("#### Performance")
            st.metric("Response Time", "< 2s", delta="Optimal")
            st.metric("Uptime", "99.9%", delta="24/7")
            st.metric("Server", "AWS EC2", delta="US-East-1")
        
        st.markdown("---")
        
        # Detailed Status Indicators
        st.markdown("#### 🔍 Detailed Component Status")
        ...
        status_data = {
            "Component": [
                ...
                "💾 Database (JSON)",
                "🌐 API Gateway (FMP)",
                "📈 Chart Renderer (Plotly)",
                "🔄 Real-time Updates"
            ],
            "Status": [
                "🟢 Active" if PREMIUMVER_IMPORTED else "🔴 Inactive",
                "🟢 Active" if ENHPROG_IMPORTED else "🔴 Inactive",
                "🟢 Active" if FULL_BACKEND else "🟡 Limited",
                "🟢 Active",
                "🟢 Active",
                "🟢 Connected" if FMP_API_KEY else "🟡 Offline",
                "🟢 Active",
                "🟢 Active"
            ],
            "Version": [
                "2.0.0", "2.5.1", "1.8.0", "1.2.0", "Built-in", "v4", "5.24.0", "1.0.0"
            ],
            "Load": [
                "15%", "25%", "10%", "5%", "2%", "8%", "12%", "18%"
            ]
        }
    ...
    with tabs[1]:
        ...
        with col1:
            ...
            chart_theme = st.selectbox(
                "Chart Color Theme",
                ["Plotly Dark", "Plotly", "Seaborn", "Simple White"],
                help="Select visualization color scheme"
            )
            
            # Default timeframe
            default_timeframe = st.selectbox(
                "Default Timeframe for Charts",
                ["1 Day", "5 Days", "1 Month", "3 Months", "6 Months", "1 Year", "All Time"],
                index=3
            )
            ...
            chart_height = st.slider(
                ...
                value=500,
                step=50,
                help="Adjust default chart height"
            )
            
            # Animation
            enable_animations = st.checkbox("Enable Chart Animations", value=True)
        ...
        with col2:
            ...
            compact_mode = st.checkbox("Compact Mode", value=False, 
                                      help="Reduce spacing and padding")
            ...
            auto_refresh = st.checkbox("Auto-refresh Data", value=False,
                                      help="Automatically refresh market data every 5 minutes")
            
            if auto_refresh:
                refresh_interval = st.slider("Refresh Interval (minutes)", 1, 30, 5)
            
            # Decimal places
            decimal_places = st.slider("Price Decimal Places", 2, 6, 4)
        ...
        with col1:
            ...
            email_notifications = st.checkbox("Email Notifications (Coming Soon)", disabled=True)
            prediction_alerts = st.checkbox("Prediction Completion Alerts", value=True)
            daily_summary = st.checkbox("Daily Summary", value=False)
        
        with col2:
            st.markdown("#### Data Display")
            show_confidence = st.checkbox("Show Confidence Intervals", value=True)
            show_technical_indicators = st.checkbox("Show All Technical Indicators", value=True)
            show_volume = st.checkbox("Show Volume on Charts", value=True)
        
        if st.button("💾 Save Display Preferences"):
            st.success("✅ Preferences saved successfully!")
            st.info("Note: Some preferences will apply on next page load")
        ...
        with col1:
            ...
            if st.session_state.is_premium:
                ...
                <div class="info-card info-card-green">
                    ...
                    <ul>
                        <li>Unlimited predictions per day</li>
                        <li>Access to all 8 ML models</li>
                        <li>Full ensemble predictions</li>
                        <li>Advanced analytics & backtesting</li>
                        <li>Cross-validation suite</li>
                ...
                with usage_col1:
                    total_predictions = st.session_state.user_stats.get('total_predictions', 0)
                    st.metric("Total Predictions", total_predictions, delta="+5 today")
                
                with usage_col2:
                    cv_runs = st.session_state.user_stats.get('cv_runs', 0)
                    st.metric("CV Runs", cv_runs, delta="+2 today")
                
                with usage_col3:
                    training_runs = st.session_state.user_stats.get('training_runs', 0)
                    st.metric("Training Runs", training_runs, delta="+1 today")
                ...
                # Usage progress bar
                predictions_used = st.session_state.free_predictions_today
                max_predictions = FREE_TIER_CONFIG['max_predictions_per_day']
                
                st.markdown(f"**Daily Predictions:** {predictions_used} / {max_predictions}")
                st.progress(predictions_used / max_predictions if max_predictions > 0 else 0)
                st.markdown(f"**Remaining today:** {remaining}")
                ...
                <div class="info-card info-card-amber">
                    ...
                    <ul>
                        <li>2 predictions per day</li>
                        <li>Access to 2 basic ML models (XGBoost, Sklearn)</li>
                        <li>3 training lab runs per day</li>
                        <li>3 CV runs per day (max 3 folds)</li>
    ...
    with tabs[3]:
        ...
        with col1:
            ...
            default_model = st.selectbox(
                ...
                ["Ensemble (All Models)", "XGBoost", "Random Forest", "LSTM", "Transformer", "Prophet"],
                help="Model used for quick predictions"
            )
        ...
        with col2:
            ...
            # Historical data
            historical_data_range = st.selectbox(
            ...
            # Data quality
            filter_outliers = st.checkbox("Filter Statistical Outliers", value=True)
            
        st.markdown("---")
        
        st.markdown("#### 🗃️ Data Export & Backup")
        
        col1, col2, col3 = st.columns(3)
        
        with col1:
            if st.button("📥 Export Predictions History"):
        ...
        with col3:
            if st.button("🔄 Reset All Settings"):
                st.warning("This will reset all preferences to default")
        ...
        with col1:
            two_factor_auth = st.checkbox("Enable Two-Factor Authentication (Coming Soon)", disabled=True)
            session_timeout = st.slider("Session Timeout (minutes)", 15, 480, 60)
        
        with col2:
            data_sharing = st.checkbox("Anonymous Usage Analytics", value=True,
                                       help="Help us improve by sharing anonymous usage data")
            crash_reports = st.checkbox("Automatic Crash Reports", value=True)
    
    # =============================================================================
    # TAB 5: ABOUT
    # =============================================================================
    with tabs[4]:
        st.markdown("### ℹ️ About QuantLearn Studio")
        ...
        with col1:
            st.markdown("""
            <div class="info-card info-card-purple">
                <h4>🎓 QuantLearn Studio</h4>
                <p><strong>Version:</strong> 2.0.0 (Build 2025.12)</p>
                <p><strong>Release Date:</strong> December 2025</p>
                <p><strong>Platform:</strong> AWS EC2 (US-East-1)</p>
                <br>
                <p>
                A comprehensive AI-powered trading education platform featuring 8 advanced machine learning models,
                real-time market data integration, interactive training labs, and professional-grade analytics.
                </p>
                <br>
                <p><strong>Key Technologies:</strong></p>
                <ul>
                    <li>🐍 Python 3.11</li>
                    <li>🧠 PyTorch 2.5.1 (CPU)</li>
                    <li>📊 Streamlit 2.0</li>
                    <li>📈 Plotly 5.24</li>
                    <li>🔬 Scikit-learn 1.8</li>
                    <li>⚡ XGBoost 2.1</li>
                    <li>🌐 Real-time FMP API</li>
            ...
            **Version 2.0.0** *(Current)*
            - ✅ Full EC2 deployment with 24/7 uptime
            - ✅ PyTorch CPU integration (175MB optimized)
            - ✅ 8 ML models fully operational
            - ✅ Real-time FMP API integration
            - ✅ Enhanced UI/UX with dark theme
            - ✅ Premium key encryption system
        ...
        with col2:
            ...
            system_info = {
                ...
                "Value": [
                    "8",
                    "Active",
                    str(st.session_state.user_stats.get('total_predictions', 0)),
                    "99.9%",
                    "Low",
                    "🟢 Online" if FMP_API_KEY else "🟡 Offline"
                ]
            }
            ...
            st.markdown("📧 Support: support@quantlearn.com")
            st.markdown("📚 Documentation: [Coming Soon]")
            st.markdown("🐛 Report Bug: [GitHub Issues]")
            st.markdown("💡 Feature Request: [Feedback Form]")
            
            st.markdown("---")
            
            st.markdown("#### 📄 Legal")
            st.markdown("© 2025 QuantLearn Studio")
