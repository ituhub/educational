...
def check_can_predict() -> Tuple[bool, str, Dict]:
    """Check if current user can make a prediction"""
    if not st.session_state.get('is_premium', False):
        return False, "Please activate a premium key first", {}
...
def record_prediction_usage(ticker: str = None, model: str = None) -> Dict:
    ...
    if st.session_state.get('is_admin', False):
        ...
        st.session_state.user_stats['xp'] = st.session_state.user_stats.get('xp', 0) + 50
        st.session_state.user_stats['level'] = st.session_state.user_stats.get('xp', 0) // 500 + 1
        return {"success": True, "remaining": float('inf'), "is_admin": True}
...
def generate_simulated_prediction(ticker: str, model_id: str) -> Dict:
    ...
    st.session_state.user_stats['level'] = st.session_state.user_stats['xp'] // 500 + 1
...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        level = st.session_state.user_stats['level']
        xp_progress = (xp % 500) / 500 * 100
        
        st.markdown(f"""
        <div class="xp-card">
            <div style="font-size: 32px; margin-bottom: 8px;">🏆</div>
            <div style="font-size: 24px; font-weight: 700;">Level {level}</div>
            <div style="font-size: 14px; opacity: 0.9;">{xp} XP Total</div>
            <div class="progress-container" style="margin-top: 12px; background: rgba(255,255,255,0.2);">
                <div class="progress-fill" style="width: {xp_progress}%; background: white;"></div>
            </div>
            <div style="font-size: 11px; margin-top: 4px; opacity: 0.8;">{int(xp % 500)}/500 to Level {level + 1}</div>
        </div>
        """, unsafe_allow_html=True)
...
def render_dashboard():
    ...
    with col1:
        ...
        <div class="metric-card">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <span class="free-badge">+{st.session_state.user_stats['predictions'] * 50} XP</span>
            </div>
    ...
    with col3:
        ...
        <div class="metric-card metric-card-warning">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span style="font-size: 24px;">🏆</span>
                <span class="premium-badge">{st.session_state.user_stats['xp']} XP</span>
            </div>
            <div style="font-size: 32px; font-weight: 700; margin: 12px 0;">Level {st.session_state.user_stats['level']}</div>
            <div style="color: #94a3b8;">Current Level</div>
        </div>
        """, unsafe_allow_html=True)
...
def check_free_prediction_limit():
    """Check if free user can make more predictions today"""
    # Reset counter if it's a new day
    today = datetime.now().strftime('%Y-%m-%d')
    if st.session_state.get('free_predictions_date') != today:
        st.session_state.free_predictions_date = today
        st.session_state.free_predictions_today = 0
    ...
    remaining = FREE_TIER_CONFIG['max_predictions_per_day'] - st.session_state.free_predictions_today
    return remaining > 0, remaining


def increment_free_prediction_count():
    """Increment free prediction counter"""
    if not st.session_state.is_premium:
        st.session_state.free_predictions_today += 1
        ...
        if not can_predict and not st.session_state.is_premium:
            ...
            if is_model_locked:
                ...
                with st.spinner("🔄 Running AI analysis..." + (" with all models" if use_ensemble else "")):
                    ...
                    st.success(f"✅ Prediction complete! +50 XP")
                    st.rerun()
...
def render_training_labs():
    ...
    with col2:
        st.metric("Total XP", f"{st.session_state.user_stats.get('xp', completed * 100):,}")
    with col3:
        st.metric("Level", st.session_state.user_stats.get('level', completed // 4 + 1))
    with col4:
    ...
    with col1:
        ...
        if st.button("🚀 Start Training", use_container_width=True, key="start_training"):
            with st.spinner(f"Training {model_options[selected_model]}..."):
                ...
                st.success(f"✅ {model_options[selected_model]} trained successfully! +150 XP")
                st.session_state.user_stats['xp'] = st.session_state.user_stats.get('xp', 0) + 150
    ...
    with col1:
        ...
        if st.button("🔬 Run Cross-Validation", use_container_width=True, key="run_cv"):
            with st.spinner("Running cross-validation..."):
                ...
                st.session_state.completed_modules.add(module_key)
                st.session_state.user_stats['xp'] = st.session_state.user_stats.get('xp', 0) + 100
...
def render_explainability_lab(lab):
    ...
    if st.button("✅ Complete Explainability Lab", key="complete_explain"):
        for module in lab['modules']:
            ...
            st.session_state.completed_modules.add(module_key)
        st.session_state.user_stats['xp'] = st.session_state.user_stats.get('xp', 0) + 200
        st.success("🎉 Explainability Lab completed! +200 XP")
...
def render_feature_engineering_lab(lab):
    ...
    if st.button("✅ Complete Feature Engineering", key="complete_features"):
        ...
        st.session_state.completed_modules.add(module_key)
        st.session_state.user_stats['xp'] = st.session_state.user_stats.get('xp', 0) + 100
        st.success(f"🎉 {category['title']} completed! +100 XP")
        ...
        with col3:
            if not is_done:
                if st.button("▶️ Start", key=f"start_{module_key}"):
                    st.session_state.completed_modules.add(module_key)
                    st.session_state.user_stats['xp'] = st.session_state.user_stats.get('xp', 0) + 100
                    st.success(f"✅ {module['name']} completed! +100 XP")
                    st.rerun()
            else:
                st.markdown("✓ Done")
    ...
    with col1:
        ...
        if st.button("🔬 Run Cross-Validation", use_container_width=True):
            with st.spinner("Running cross-validation..."):
                ...
                st.session_state.user_stats['xp'] = st.session_state.user_stats.get('xp', 0) + 100
                st.success("✅ Cross-validation complete! +100 XP")
                st.rerun()
    ...
    with col1:
        ...
        if st.button("▶️ Run Backtest", use_container_width=True):
            with st.spinner("Running backtest..."):
                ...
                st.session_state.user_stats['xp'] += 75
                st.success("✅ Backtest complete! +75 XP")
                st.rerun()
...
def render_how_it_works():
    ...
    if 'education_progress' in st.session_state:
        ...
        st.session_state.user_stats['xp'] += 25
...
def render_settings():
    ...
    with col1:
        ...
        st.markdown("### 🔑 Premium Keys")
        st.markdown("**Available Keys (20 predictions each):**")
        st.code("""ALPHA-TRADER-2024
BETA-INVEST-2024
GAMMA-ANALYST-24
DELTA-QUANT-2024
EPSILON-STRAT-24""")
        st.markdown("**Admin Key (unlimited):**")
        st.code("ADMIN-MASTER-2024")
        st.caption("Enter any key in the sidebar to unlock premium features")
