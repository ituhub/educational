...
def create_enhanced_prediction_section():
    ...
    # Main prediction controls - MODIFIED: Removed cross-validation button
    col1, col2 = st.columns([3, 1])  # Changed from 3 columns to 2
    ...
    # REMOVED: Cross-validation button section
    # with col2:
    #     if st.session_state.subscription_tier == 'premium':
    #         cv_button = st.button("📊 Cross-Validate", help="Run cross-validation")
    #     else:
    #         cv_button = False
    ...
    # Set cv_button to False since we removed it
    cv_button = False
    
    # PREDICTION EXECUTION with click tracking
    if predict_button:
        # ... existing prediction code remains the same ...
    ...
    # Cross-validation removed - no longer executed
    # if cv_button:
...
def _create_free_tier_sidebar(advanced_app_state):
    """
    Create sidebar content for free tier with premium activation.
    ...
    premium_key = st.text_input(
        ...
        help="Enter 'Prem246_357' for full access including User Management"
    )
    
    if st.button("🚀 Activate Premium", type="primary", key="activate_premium_button"):
        # Use the advanced app state update method
        success = advanced_app_state.update_subscription(premium_key)
        
        if success:
            st.success("Premium activated successfully!")
            st.info("🎉 User Management tab is now available!")
            time.sleep(1)
            st.rerun()
        else:
            st.error("Invalid premium key. Use 'Prem246_357' for full access.")
...
def create_main_content():
    ...
    if st.session_state.subscription_tier == 'premium':
        ...
        if has_master_key:
            # Master key user gets admin panel
            tabs = st.tabs([
                "🎯 AI Prediction", 
                "📊 Advanced Analytics", 
                "💼 Portfolio Management", 
                "📈 Backtesting",
                "🔧 Admin Panel"  # Only for master key
            ])
            ...
            # Regular premium users don't get admin panel
            tabs = st.tabs([
                "🎯 AI Prediction", 
                "📊 Advanced Analytics", 
                "💼 Portfolio Management", 
        ...
        # Free tier tabs remain the same
        tabs = st.tabs([
            "🎯 AI Prediction", 
            "📊 Basic Analytics"
        ])
